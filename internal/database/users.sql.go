...
func (q *Queries) UpdateUserAuth(ctx context.Context, arg UpdateUserAuthParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateUserAuth, arg.Email, arg.HashedPassword, arg.ID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
...
type UpdateUserMembershipRow struct {
	ID          uuid.UUID
	CreatedAt   time.Time
	UpdatedAt   time.Time
	Email       string
...
func (q *Queries) UpdateUserMembership(ctx context.Context, arg UpdateUserMembershipParams) (UpdateUserMembershipRow, error) {
	row := q.db.QueryRowContext(ctx, updateUserMembership, arg.IsChirpyRed, arg.ID)
	var i UpdateUserMembershipRow
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
		&i.UpdatedAt,
